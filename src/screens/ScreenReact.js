...
const ScreenReact = () => {
  ...
  const getQuestions = async page => {
    try {
      ...
      const response = await fetch(
        `https://api.stackexchange.com/search/advanced?site=stackoverflow.com&q=react.js&page=${page}&pagesize=10`,
      );
  ...
  return (
    <View style={{flex: 1}}>
      <Header title={' React Js Questions'} />
      <FlatList
        data={data || []}
        renderItem={({item}) => {
          return (
