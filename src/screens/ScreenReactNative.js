...
const ScreenReactNative = () => {
  ...
  const getQuestions = async page => {
    try {
      ...
      const response = await fetch(
        `https://api.stackexchange.com/search/advanced?site=stackoverflow.com&q=reactnative&page=${page}&pagesize=10`,
      );
  ...
  return (
    <View style={{flex: 1}}>
      <Header title={' React native Questions'} />
      <FlatList
        data={data || []}
        renderItem={({item}) => {
          return (
...
export default ScreenReactNative;
